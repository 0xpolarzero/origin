...
export const SettingsModels: Component = () => {
  ...
  const importOpenCodeModelToggles = async () => {
    setLoading(true)
    await readOpenCodeModelSource()
      .then((raw) => {
        const parsed = parseOpenCodeModelToggles(raw)
        if (parsed.status === "error") {
          const description =
            parsed.reason === "missing"
              ? "OpenCode model toggles source was not found."
              : "OpenCode model toggles source is invalid."
          showToast({
            title: "Load OpenCode model toggles",
            description,
          })
          return
        }
        ...
        parsed.entries.forEach((entry) => {
          models.setVisibility({ providerID: entry.providerID, modelID: entry.modelID }, entry.visible)
        })
        ...
        showToast({
          ...
          icon: "circle-check",
          title: "Load OpenCode model toggles",
          description: `${parsed.entries.length} model toggles imported from OpenCode.`,
        })
      })
      .catch((error: unknown) => {
