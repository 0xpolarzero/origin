...
function env() {
  ...
  return uniq(
    Object.entries(value)
      .filter(([key, item]) => Boolean(item) && secret(key))
      .map(([, item]) => item ?? ""),
  )
}

function encode_base64(value: string) {
  try {
    const buffer = (globalThis as { Buffer?: { from(input: string): { toString(encoding: string): string } } }).Buffer
...
function variants(value: string) {
  ...
  const base64 = encode_base64(value)
