...
- `docs/specs/01.native-app-pruning.spec.md` (spec source document)
- `docs/specs/01.native-app-pruning.log.md` (this log)

## Deviations from Spec

- Validation could not fully pass in this environment because `packages/opencode` build requires network access to `https://models.dev/api.json`.
- As a result, `bun run dev:desktop` remained blocked due to the same predev sidecar build dependency.
